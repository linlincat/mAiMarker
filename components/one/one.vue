<template>
  <div>
    ...
    <button @click="removeChildP"> 移除辅助线 </button>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      width: 600,
      uuid: '0da9130',
      currentImage: require("../../src/assets/mao.jpg")  // url 或者 base64
    }
  },
  mounted() {
    ...
    document.onkeydown = (event) => {
      ...
      if (ctrlKey && e.keyCode == 87) { // 按 W 开始标记
        this.toRect();
      }
      if (ctrlKey && e.keyCode == 67) { // 按 C 清除辅助线
        this.removeChildP();
      }
      if (ctrlKey && e.keyCode == 76) { // 按 L 显示辅助线
        this.showCross();
      }
      if (e && e.keyCode == 69) { // 按 E 结束标记
        this.cancelRect()
      } 
    };
  ...
  methods: {
    removeChildP() {
      var ni = document.getElementById('mlin').getElementsByTagName("p");
      var box = document.getElementById("mlin");
      for (let i = 0; i < 2; i++) {
        box.removeChild(ni[0]);
      }
    },
    showCross() {
